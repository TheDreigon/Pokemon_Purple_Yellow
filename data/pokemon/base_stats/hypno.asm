	db DEX_HYPNO ; pokedex id
	base_stat_row 85, 75, 70, 105, 65 ; hp atk def spc spd
	db PSYCHIC_TYPE, DARK ; type
	db 75 ; catch rate
	db 165 ; base exp

	INCBIN "gfx/pokemon/front/hypno.pic", 0, 1 ; sprite dimensions
	dw HypnoPicFront, HypnoPicBack

	db TACKLE, HYPNOSIS, NO_MOVE, NO_MOVE ; level 1 learnset
	db GROWTH_MEDIUM_FAST ; growth rate

	; tm/hm learnset
	tmhm BODY_SLAM,    CALM_MIND,    DARK_PULSE,   DOUBLE_EDGE,  EXTRASENSORY, \
	     FAKE_TEARS,   NIGHT_SHADE,  PSYCHIC_M,    REFLECT,      SCARY_FACE, \
	     SHADOW_BALL,  TAKE_DOWN,    MIMIC,        MIRROR_MOVE,  SUBSTITUTE, \
	     METRONOME
	; end

	db 0 ; padding
