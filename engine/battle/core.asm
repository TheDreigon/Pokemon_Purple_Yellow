...
; CalcHitChance moved to engine/battle/hit_and_penalties.asm ("Battle Effects" section).
...
ApplyBurnAndParalysisPenalties:
	ldh [hWhoseTurn], a
	farcall HalveSpeedDueToParalysis
	farjp HalveAttackDueToBurn
...
; v0.7 hard mode knob #17 (Forte, 2026-09-01): badges only strengthen
...
; BattleRandom moved to home/battle_random.asm. It is called 24 times from
; here and 23 times from engine/battle/effects.asm, which is about to live in
; another bank -- and the home bank is mapped at all times, so both sides keep
; a plain `call` and neither pays a farcall's clobbered registers.
...
; play SPECIAL battle animation index a (1..NUM_SPECIAL_ANIMS) - the anim
; split's counterpart of PlayMoveAnimation (v0.7, 2026-08-31).
	ldh [hSpecialAnimIndex], a
	ld a, SPECIAL_ANIM_MARKER
	jr PlayMoveAnimation
